...
_redact_secrets_filter = _SecretDisplayRedactor()
install_event_filter(LOG)


def install_uvicorn_log_redaction() -> None:
    configure_logging(load_options().get("log_level"))
...
def load_state() -> None:
    ...
    try:
        ...
        try:
            ...
            LOG.warning('state_file_corrupted_backed_up_and_reset')
        except Exception as exc:
            LOG.warning('state_file_corrupted_backup_failed_and_reset')
        loaded = {}
    except Exception as exc:
        LOG.warning('state_file_unreadable_reset')
        loaded = {}
...
def update_state(mutator) -> bool:
    with _state_lock:
        ...
        if not valid_pairing_transition(previous_pairing_state, next_pairing_state):
            LOG.warning('invalid_pairing_transition')
        _state.clear()
...
def record_proxy_status(
    ...
    *,
    ...
    if bucket is None:
        return
    # Diagnostics can be exported for support; retain no URLs or exception text.
    payload = {
        "status_code": status_code if type(status_code) is int and 100 <= status_code <= 599 else None,
        "source": source if source in {"upstream", "proxy"} else "unknown",
        "observed_at": now_iso(),
    }
    with _proxy_diag_lock:
        _proxy_diagnostics["last_upstream_status"][bucket] = payload
        if status_code == 401:
            _proxy_diagnostics["last_401"] = {"path": bucket, **payload}
...
def invalidate_runtime_caches(reason: str) -> None:
    ...
    LOG.debug('runtime_caches_invalidated')
...
def collect_netbird_status() -> Dict[str, Any]:
    ...
    with _netbird_status_lock:
        ...
        if cached is not None and now_monotonic - cached_at < NETBIRD_STATUS_CACHE_SECONDS:
            LOG.debug('netbird_status_refresh_skipped_cached')
    ...
    if rc == 0 and out:
        try:
            ...
            LOG.debug('netbird_self_identity')
            value = {
                "connected": connected,
...
def portal_request_json(
    ...
    *,
    ...
    for index, (base_url, reason) in enumerate(candidates):
        ...
        LOG.info('portal_request_destination')
        try:
            ...
            LOG.warning('portal_transport_failure')
            last_exc = exc
            last_url = url
...
def log_upstream_result(url: str, status: Optional[int], body: str) -> None:
    LOG.warning('upstream_request', {'status': status})
...
def connectivity_error_response(url: str, exc: Exception) -> JSONResponse:
    ...
    LOG.warning('upstream_connectivity')
    return JSONResponse(
        {
            "ok": False,
...
def local_network_context() -> Dict[str, Any]:
    try:
        ...
        LOG.warning('local_network_context_collection_failed')
        return {
            "source": "error",
            "local_ipv4_addresses": [],
            "local_ipv4_subnets": [],
...
def resolve_route_target(options: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if supervisor_error:
        ...
        LOG.info('route_target_supervisor_network_unavailable')
...
def refresh_route_runtime_state(options: Optional[Dict[str, Any]] = None, status: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    with _route_refresh_lock:
        ...
        if cached_value is not None and cached_key == cache_key and now_monotonic - cached_at < ROUTE_REFRESH_CACHE_SECONDS:
            LOG.debug('ha_route_refresh_skipped_cached')
    ...
    if access_mode == ACCESS_MODE_ROUTE and discovered_identity[0] and discovered_identity != current_identity and active_reachable and discovered_reachable:
        if pending_identity != discovered_identity:
            ...
            LOG.info('ha_route_target_change_observed')
        else:
            try:
    ...
    if health_status != str(route_state.get("health_status") or ""):
        changed = True
        LOG.info('ha_route_health_status_changed')
    ...
    if changed:
        ...
        LOG.info('ha_route_state_updated')
    if route_state_material_view(route_state) == route_state_material_view(next_route):
        ...
        LOG.debug('ha_route_refresh_skipped_unchanged')
    else:
...
def apply_portal_binding_validation_result(state: Dict[str, Any], response: Dict[str, Any], *, reported_at: str) -> None:
    ...
    if portal_binding_diagnostics:
        LOG.warning('portal_binding_invalidation')
...
def clear_local_identity_state(state: Dict[str, Any], *, reset_route: bool = False) -> Dict[str, bool]:
    ...
    LOG.info('local_identity_cleared')
    return fields
...
def portal_reset_cleanup_result(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        LOG.info('portal_reset_network_error')
        return {"attempted": True, "ok": False, "result": "network_error"}
    except Exception as exc:
        LOG.info('portal_reset_invalid_response')
...
def refresh_runtime_state() -> Dict[str, Any]:
    ...
    if stable_json_dumps(state) == stable_json_dumps(original):
        LOG.debug('runtime_state_refresh_skipped_unchanged')
        return state
    write_json_file(STATE_PATH, state)
    with _state_lock:
        _state.clear()
...
def netbird_up(state: Dict[str, Any], options: Dict[str, Any]) -> tuple[bool, str]:
    ...
    LOG.info('netbird_dns_configuration')
    if binding.get("status") in {"missing", "invalid"}:
        LOG.warning('missing_binding_before_netbird_up')
    ...
    if rc == 0:
        ...
        LOG.info('netbird_peer_startup_complete')
        return True, "ok"
    ...
    LOG.warning('netbird_peer_startup_failed')
...
def send_heartbeat(status: Dict[str, Any], state: Dict[str, Any], options: Dict[str, Any]) -> Optional[str]:
    ...
    if not device_token or not device_id or not home_id:
        LOG.debug('ha_route_heartbeat_skipped_missing_credentials')
        return None
    ...
    if not binding_id:
        LOG.warning('ha_route_heartbeat_skipped_missing_binding_id')
        update_state(
    ...
    if not peer_id:
        try:
            ...
            if peer_id:
                ...
                LOG.info('device_config_recovered_netbird_management_peer_id')
        except urllib.error.HTTPError as exc:
            ...
            LOG.warning('device_config_fetch_failed')
        except Exception as exc:
            LOG.warning('device_config_fetch_failed')
    if not peer_id:
        LOG.warning('ha_route_heartbeat_skipped_netbird_management_peer_id_missing')
        update_state(
            lambda current: current.update(
                {
    ...
    if (
        ...
        LOG.debug('ha_route_heartbeat_skipped_unchanged')
        return None
    ...
    LOG.info('ha_route_heartbeat_queued')
    LOG.info('binding_presented_on_heartbeat')
    try:
        ...
        LOG.info('ha_route_heartbeat_success')
        return None
    except urllib.error.HTTPError as exc:
        failed_url = getattr(exc, "_hometunnel_url", None)
        body = read_http_error_body(exc)
        ...
        LOG.warning('heartbeat_send_failed')
        update_state(
            lambda current: current.update(
                {
                    **(
...
def persist_netbird_agent_status(status: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bool(latest_state.get("agent_running")) is True and dict(latest_state.get("netbird") or {}) == status:
        LOG.debug('netbird_agent_state_persistence_skipped_unchanged')
        return latest_state
    ...
    try:
        while True:
            ...
            if bool(state.get("netbird_transport_degraded")) != degraded_active:
                ...
                write_json_file(STATE_PATH, state)
                LOG.log(logging.WARNING if degraded_active else logging.INFO, 'netbird_transport_degraded_state_changed')
            ...
            recovery_state = normalize_optional_string(state.get("recovery_state"))
            if recovery_state in NON_RETRYABLE_RECOVERY_STATES:
                LOG.info('ha_route_recovery_paused')
            ...
            if status.get("connected"):
                if bool(route_state.get("needs_report")) or desired_routes != applied_routes:
                    LOG.info('ha_route_update_pending_portal_reconciliation')
                ...
                time.sleep(POLL_INTERVAL_SECONDS)
                continue
            LOG.info('ha_route_heartbeat_skipped_netbird_not_connected')
...
def ui_refresh_loop() -> None:
    while not _stop_event.is_set():
        ...
        if bool(state.get("agent_running")):
            LOG.debug('ui_refresh_live_status_skipped_while_netbird_agent_loop_is_active')
        else:
            state = persist_runtime_observations(load_options())
            if not state:
                LOG.debug('ui_refresh_persistence_skipped_empty_state')
...
def run_pairing_start_request(local_device_id: str, started_at: str) -> None:
    try:
        ...
        LOG.warning('device_auth_start_background_request_failed')
...
class HomeAssistantUpstreamResolver:
    ...
    def _candidate_urls(self) -> list[tuple[str, str]]:
        ...
        # Only internal service names or explicitly configured administrator
        # destinations may receive HA credentials. Shared LAN mDNS is not identity.
    ...
    def resolve(self, force: bool = False, reason: str = "request") -> str:
        ...
        for source, candidate in self._candidate_urls():
            ...
            if healthy:
                resolved_at = now_iso()
                LOG.info('ha_proxy_upstream_selected')
                with self._lock:
            ...
            LOG.warning('ha_proxy_upstream_rejected')
    ...
    if _ui_trusted_networks is None:
        ...
        for entry in entries:
            try:
                ...
                LOG.warning('ui_trusted_client_invalid')
...
async def enforce_ingress_only_access(request: Request, call_next):
    ...
    if not ui_client_is_trusted(client_host):
        LOG.warning('ui_request_blocked')
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function refresh() {
    try {
      ...
      console.error(JSON.stringify({event: "ui_status_refresh_failed"}));
  ...
  async function startAuth() {
    try {
      ...
      startInFlight = false;
      authError = error;
      console.error(JSON.stringify({event: "ui_auth_start_failed"}));
  ...
  async function pollAuth() {
    ...
    try {
      ...
      authError = error;
      console.error(JSON.stringify({event: "ui_auth_poll_failed"}));
  ...
  startBtn.addEventListener("click", () => {
    console.log(JSON.stringify({event: "ui_pairing_clicked"}));
    void startAuth();
  });
...
def startup_event() -> None:
    ...
    log_method('supervisor_api_capability', {'received': capability['received']})
    load_state()
...
def api_ping() -> Dict[str, Any]:
    LOG.info('get_api_ping')
    return {"ok": True, "message": "pong"}


@app.get("/api/status")
def api_status() -> Dict[str, Any]:
    LOG.info('get_api_status')
    global _options
    _options = load_options()
...
def api_binding_status() -> Dict[str, Any]:
    LOG.info('get_api_binding_status')
    state_copy = redact_state(refresh_runtime_state())
    binding = dict(state_copy.get("binding") or {})
    return {
        "ok": True,
...
async def api_auth_poll(request: Request) -> JSONResponse:
    ...
    LOG.warning('upstream_request', {'status': 200})
    ...
    LOG.info('mixed_identity_detected_false')
...
def api_pairing_reset() -> JSONResponse:
    LOG.info('post_api_pairing_reset')
    invalidate_runtime_caches("pairing_reset")
    state_snapshot = read_state_snapshot()
    LOG.info('pairing_reset_started')
...
def api_agent_restart() -> JSONResponse:
    LOG.info('post_api_agent_restart')
...
def start_proxy_resolver_loop() -> None:
    while not _proxy_stop_event.wait(PROXY_RESOLVE_INTERVAL_SECONDS):
        try:
            ...
            LOG.warning('ha_proxy_periodic_resolve_failed')
...
async def stream_proxy_request(request: Request, upstream_base: str, upstream_url: str) -> StreamingResponse:
    ...
    if should_debug_auth_flow(path):
        LOG.info('ha_proxy_auth_request')
...
async def proxy_http_request(request: Request, full_path: str = "") -> Response:
    ...
    try:
        ...
        if path == "/auth/authorize":
            LOG.warning('ha_proxy_auth_response')
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            last_error = exc
            LOG.warning('ha_proxy_connection_failure')
            ...
            LOG.warning('ha_proxy_request_error')
...
async def proxy_websocket_session(websocket: WebSocket, full_path: str) -> None:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            LOG.warning('ha_proxy_websocket_resolve_failed')
            await websocket.close(code=1013, reason="Home Assistant upstream unavailable")
            return
        except (OSError, websockets.WebSocketException) as exc:
            LOG.warning('ha_proxy_websocket_connect_failure')
            if upstream_base:
                await asyncio.to_thread(_proxy_resolver.invalidate, upstream_base, str(exc))
            if attempt >= attempts:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3 and sys.argv[1] == "--check-bind":
        try:
            ...
            LOG.error("proxy_bind_failed")
            raise SystemExit(1)
        raise SystemExit(0)
    if len(sys.argv) > 1 and sys.argv[1] == "--ha-proxy":
        uvicorn.run(proxy_app, host=PROXY_LISTEN_HOST, port=PROXY_LISTEN_PORT, log_config=None, access_log=False, proxy_headers=False)
        raise SystemExit(0)
    uvicorn.run(app, host=LISTEN_HOST, port=LISTEN_PORT, log_config=None, access_log=False, proxy_headers=False)
